...
function applyVideoCards(data) {
  ...
  for (let i = 0; i < videoDataArray.length; i++) {
    ...
    if (videoDataArray[i].en) {
      ...
      newVideoLocationEn.innerHTML = videoDataArray[i].en.location;
      newVideoDateEn.innerHTML = videoDataArray[i].en.date;
      newVideoCreditEn.innerHTML = videoDataArray[i].en.credit;
    ...
    if (videoDataArray[i].es) {
      ...
      let newVideoLocationEs = newVideoCardEs.querySelector(
        ".es-js-video-ui-location"
      );
      let newVideoDateEs = newVideoCardEs.querySelector(".es-js-video-ui-date");
      let newVideoCreditEs = newVideoCardEs.querySelector(
        ".es-js-video-ui-credit"
      );
